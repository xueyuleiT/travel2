...
public class Banner<T, BA extends BannerAdapter> extends FrameLayout implements BannerLifecycleObserver {
    ...
    private float mBannerLTRadius = 0;
    private float mBannerLBRadius = 0;
    private float mBannerRTRadius = 0;
    private float mBannerRBRadius = 0;
    ...
    private void init(Context context) {
        ...
        if (bannerHeight == 0) {
            ...
            mViewPager2.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, bannerHeight));
        }
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        ...
        if (mBannerRadius > 0) {
            mBannerLTRadius = mBannerRadius;
            mBannerLBRadius = mBannerRadius;
            mBannerRTRadius = mBannerRadius;
            mBannerRBRadius = mBannerRadius;
        }else {
            mBannerLTRadius = a.getDimensionPixelSize(R.styleable.Banner_banner_lt_radius, 0);
            mBannerLBRadius = a.getDimensionPixelSize(R.styleable.Banner_banner_lb_radius, 0);
            mBannerRTRadius = a.getDimensionPixelSize(R.styleable.Banner_banner_rt_radius, 0);
            mBannerRBRadius = a.getDimensionPixelSize(R.styleable.Banner_banner_rb_radius, 0);
        }
        ...
        bannerHeight = a.getDimensionPixelSize(R.styleable.Banner_banner_height, 0);
...
//            }
    ...
    protected void dispatchDraw(Canvas canvas) {
        if (mBannerRadius > 0) {
            ...
            canvas.restore();
        }else if (mBannerLTRadius > 0 || mBannerLBRadius > 0 || mBannerRTRadius > 0 || mBannerRBRadius > 0) {
            canvas.saveLayer(new RectF(0, 0, canvas.getWidth(), canvas.getHeight()), mImagePaint, Canvas.ALL_SAVE_FLAG);
            super.dispatchDraw(canvas);
            //绘制外圆环边框圆环
            drawTopLeft(canvas);
    ...
    private void drawTopLeft(Canvas canvas) {
        Path path = new Path();
        path.moveTo(0, mBannerLTRadius);
        path.lineTo(0, 0);
        path.lineTo(mBannerLTRadius, 0);
        path.arcTo(new RectF(0, 0, mBannerLTRadius * 2, mBannerLTRadius * 2),
                -90, -90);
        path.close();
    ...
    private void drawTopRight(Canvas canvas) {
        int width = getWidth();
        Path path = new Path();
        path.moveTo(width - mBannerRTRadius, 0);
        path.lineTo(width, 0);
        path.lineTo(width, mBannerRTRadius);
        path.arcTo(new RectF(width - 2 * mBannerRTRadius, 0, width,
                mBannerRTRadius * 2), 0, -90);
        path.close();
        canvas.drawPath(path, mRoundPaint);
    }
    ...
    private void drawBottomLeft(Canvas canvas) {
        ...
        Path path = new Path();
        path.moveTo(0, height - mBannerLBRadius);
        path.lineTo(0, height);
        path.lineTo(mBannerLBRadius, height);
        path.arcTo(new RectF(0, height - 2 * mBannerLBRadius,
                mBannerLBRadius * 2, height), 90, 90);
        path.close();
        canvas.drawPath(path, mRoundPaint);
    }
    ...
    private void drawBottomRight(Canvas canvas) {
        ...
        int width = getWidth();
        Path path = new Path();
        path.moveTo(width - mBannerRBRadius, height);
        path.lineTo(width, height);
        path.lineTo(width, height - mBannerRBRadius);
        path.arcTo(new RectF(width - 2 * mBannerRBRadius, height - 2
                * mBannerRBRadius, width, height), 0, 90);
        path.close();
        canvas.drawPath(path, mRoundPaint);
    }
